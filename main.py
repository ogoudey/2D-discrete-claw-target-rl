...
#        print("(Reset)", self.complete["robot_state"]["position"])
    ...
    def observe(self):
        rs = self.complete['robot_state']['position']
        os = self.complete['object_state']['position']
        nparray = np.concatenate((rs, os))
    ...
    def act(self, action):
        try:
            if self.complete["robot_state"]["position"][0] + action[0] < 0 or self.complete["robot_state"]["position"][1] + action[1] < 0:
                raise IndexError 
            self.complete["map"][int(self.complete["robot_state"]["position"][0]) + int(action[0]), int(self.complete["robot_state"]["position"][1]) + int(action[1])] = 0
        ...
        if int(self.complete["robot_state"]["position"][0]) == self.complete["object_state"]["position"][0] and int(self.complete["robot_state"]["position"][1]) == self.complete["object_state"]["position"][1]:
            self.mem_reward = 10
        else:
            self.mem_reward = -1 * np.linalg.norm(np.array(self.complete["object_state"]["position"]) - np.array(self.complete["robot_state"]["position"]))
            self.mem_reward = 0
    ...
    try:
        while True:
            ...
            for i in range(0, episode_length):
                action = pi.sample(state)
                print("pi(",state,") ==>>", action[0].detach().numpy()) 
                s.act(action[0].detach().numpy())
                reward = s.reward()
                state = s.observe()
                
                v.render(s.complete, None)
